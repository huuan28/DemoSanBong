...
namespace DemoSanBong.Services
{
    public class MomoService : IMomoService
    {
        ...
        public async Task<MomoCreatePaymentResponseModel> CreatePaymentAsync(OrderInfoModel model, string action)
        {
            ...
            if (action == "invoice")
                rawData = $"partnerCode={_options.Value.PartnerCode}&accessKey={_options.Value.AccessKey}&requestId={model.OrderId}&amount={model.Amount}&orderId={model.OrderId}&orderInfo={model.OrderInfo}&returnUrl={_options.Value.ReturnUrlInvoice}&notifyUrl={_options.Value.NotifyUrl}&extraData=";
            ...
            if (action == "invoice")
            {
                requestData = new
                {
                    ...
                    requestType = _options.Value.RequestType,
                    notifyUrl = _options.Value.NotifyUrl,
                    returnUrl = _options.Value.ReturnUrlInvoice,
                    orderId = model.OrderId,
                    amount = model.Amount.ToString(),
                    orderInfo = model.OrderInfo,
                    requestId = model.OrderId,
        ...
        public MomoExecuteResponseModel PaymentExecuteAsync(IQueryCollection collection)
        {
            ...
            var errorCode = int.Parse(collection.First(s => s.Key == "errorCode").Value);
